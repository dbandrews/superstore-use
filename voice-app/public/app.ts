// Superstore Voice Shopping - Frontend
...
interface AppState {
  ...
  productNames: Record<string, string>; // product_code → display name
}
...
async function startSession() {
  try {
    ...
    clog(`Session start failed: ${(err as Error).message}`, "error");
...
// Current in-progress assistant message element for streaming text
...
function handleServerEvent(event: any) {
  switch (event.type) {
    ...
    case "response.done":
      // Check if the response contains tool calls that haven't been handled
...
async function callBackend(fnName: string, args: any): Promise<any> {
  ...
  // Inject stored cart_id and store_id into requests that need them
  const body: any = { ...args };
